        ...
        if (executor) {
          await executor.tryOrQueue(
            ...
            () => ({
              to: contractAddress,
              value: "0",
              data: contract.interface.encodeFunctionData("grantRole", [DEFAULT_ADMIN_ROLE, adminToKeep]),
            }),
          );
        }
      ...
      if (executor) {
        await executor.tryOrQueue(
          ...
          () => ({
            to: contractAddress,
            value: "0",
            data: contract.interface.encodeFunctionData("revokeRole", [DEFAULT_ADMIN_ROLE, adminToRevoke]),
          }),
        );
      }
      manualActions?.push(`${contractRef}.revokeRole(DEFAULT_ADMIN_ROLE, ${adminToRevoke})`);
