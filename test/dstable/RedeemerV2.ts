...
import { CollateralHolderVault, OracleAggregator, RedeemerV2, IssuerV2, TestERC20, TestMintableERC20 } from "../../typechain-types";
import { getTokenContractForSymbol, TokenInfo } from "../../typescript/token/utils";
import { createDStableFixture, DUSD_CONFIG, DStableFixtureConfig } from "./fixtures";
import { DUSD_ISSUER_V2_CONTRACT_ID } from "../../typescript/deploy-ids";
...
// Only test dUSD configuration for Ronin
  ...
  describe(`RedeemerV2 for ${config.symbol}`, () => {
    ...
    describe("Deployment and Configuration", () => {
      ...
      it("revokes withdrawer role from legacy RedeemerWithFees", async function () {
        ...
        expect(has).to.equal(false, `${legacyId} should not retain withdrawer role`);
      });
    });
