...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await transferRedeemerWithFeesRoles(hre, redeemerWithFeesContractId, deployerSigner, governanceMultisig, deployer);
...
async function transferRedeemerWithFeesRoles(
  ...
  try {
    ...
    if (redeemerWithFeesDeployment) {
      ...
      const redeemerWithFeesContract = await ethers.getContractAt("RedeemerWithFees", redeemerWithFeesDeployment.address, deployerSigner);
      ...
      // Grant roles to multisig
      if (!(await redeemerWithFeesContract.hasRole(DEFAULT_ADMIN_ROLE, governanceMultisig))) {
        await redeemerWithFeesContract.grantRole(DEFAULT_ADMIN_ROLE, governanceMultisig);
        console.log(`    ➕ Granted DEFAULT_ADMIN_ROLE to ${governanceMultisig}`);
      } else {
        console.log(`    ✓ DEFAULT_ADMIN_ROLE already granted to ${governanceMultisig}`);
      }

      if (!(await redeemerWithFeesContract.hasRole(REDEMPTION_MANAGER_ROLE, governanceMultisig))) {
        await redeemerWithFeesContract.grantRole(REDEMPTION_MANAGER_ROLE, governanceMultisig);
      ...
      console.log(`  ⚠️ ${redeemerWithFeesContractId} not deployed, skipping role transfer`);
    }
  } catch (error) {
    console.error(`  ❌ Failed to transfer ${redeemerWithFeesContractId} roles: ${error}`);
  }
